...
app.get('/api/search', async (req, res) => {
    try {
        const searchTerm = req.query.name.toLowerCase(); // Get the search query parameter

        const querySnapshot = await db.collection('dining-halls').where('name', '>=', searchTerm).get();
        const restaurants = [];
